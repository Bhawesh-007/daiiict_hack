...
type Checklist = { items?: Array<{ source_key?: string; source_name?: string; status: string; reason?: string }> };
...
export default function WorkflowPage() {
  ...
  const [message, setMessage] = useState("Choose a step to begin.");
  ...
  return <main><Nav />
    <div className="topbar"><div><p className="eyebrow">EMISSIONLENS · LAYER 1</p><h1>Assessment workspace</h1><p>Move from facility information to a reviewed source inventory, activity data, and emissions insights.</p></div><Link className="secondary-link" href="/calculations">View emissions summary →</Link></div>
    ...
    <section className="workflow-steps">
      <button className="primary-workflow" onClick={buildDemoProfile}><b>★</b><span><strong>Build complete demo profile</strong><small>Run template, rules + ML, confirm supported sources, import sample data, and calculate charts.</small></span></button>
    ...
    {checklist?.items && <section className="card workflow-data-panel"><div className="section-title"><div><p className="eyebrow">SOURCE COVERAGE</p><h2>Universal checklist</h2></div><span className="pill">{checklist.items.length} items</span></div><div className="checklist-grid">{checklist.items.map((item, index) => <div className="checklist-item" key={item.source_key || index}><span className={`status-dot ${item.status.toLowerCase()}`} /> <div><strong>{item.source_name || item.source_key || "Source family"}</strong><small>{item.status.replaceAll("_", " ")}</small></div></div>)}</div></section>}
    {candidates.length > 0 && <section className="card review-panel"><div className="section-title"><div><p className="eyebrow">SOURCE REVIEW</p><h2>Possible emission sources</h2></div><span className="pill">User decision required</span></div>{candidates.map(candidate => <article className="candidate" key={candidate.id}><div><h3>{candidate.source_name}</h3><p>{candidate.reason || "Suggested for review."}</p><small>{candidate.origin} suggestion · {candidate.suggested_scope || "Scope to review"}</small></div><div className="candidate-actions"><button onClick={() => review(candidate, "CONFIRMED")}>Confirm source</button><button className="soft-button" onClick={() => review(candidate, "MISSING_INFORMATION")}>Need info</button><button className="text-button" onClick={() => review(candidate, "NOT_APPLICABLE")}>Not applicable</button></div></article>)}</section>}
